...
import "./SafeMath.sol";
import "./SwipSwapLibrary.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library SwipSwapLiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = SwipSwapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        require(
            ...
            "SwipSwapArbitrageLibrary: ZERO_PAIR_RESERVES"
        );
        ...
        if (aToB) {
            uint256 amountOut = SwipSwapLibrary.getAmountOut(
                amountIn,
                reserveA,
                reserveB
            );
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = SwipSwapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ISwipSwapPair pair = ISwipSwapPair(
            SwipSwapLibrary.pairFor(factory, tokenA, tokenB)
        );
        bool feeOn = ISwipSwapFactory(factory).feeTo() != address(0);
